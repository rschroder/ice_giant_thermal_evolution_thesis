...
In this section, we'll review the common means of energy transport within a giant planet's interior and discuss the criteria for convection and condensation to occur.
...
\end{equation}
where $s$ is entropy, T is the temperature, and P is the pressure. This can also be expressed in terms of temperature and altitude \citep{sanchez-lavega} as:
\begin{equation}
  \frac{dT}{dz} = \frac{-g}{C_{P}} = \Gamma_{\rm dry}
\end{equation}
where z is the altitude, and $C_{P}$ is specific heat. In a region of homogeneous chemical composition, the Schwarzschild \& Harm criterion \citep{leconte_2017}, defined as
 \begin{equation}
  \nabla_{T} > \nabla_{\rm ad}.
\end{equation}
...
\end{equation}
 we use the Ledoux criterion\citep{leconte_chabrier_2013}, defined as
\begin{equation}
  \nabla_{T} > \nabla_{\rm ad} + \nabla_{\mu}
\end{equation}
to determine whether convection can occur.
\subsubsection{Moist Convection}
Condensation can have a large impact on a planet's energy balance, so it is important consider the presence of condensible species. Gases condense at at sufficiently low temperatures or high pressures. Condensation of a gas is characterized by its saturation vapor pressure, which derives from the Clausius-Clapeyron equation \citep{sanchez_2011}. The saturation vapor pressure, $P_{\rm sat}$, is given by 
\begin{equation}
...
We begin our description of the physics of our interior structure model by assuming that we are dealing with a spherically symmetric object with no electromagnetic field.  With these assumption in place, we employ the following conservation equations to describe the interior and evolution of ice giants.
...
Our interior structure model modifies the conventional structure described in Section \ref{Three-layer Model with Dry Adiabat} by adding a moist adiabatic layer to the outer envelope, which under favorable conditions, allows for the condensation of H$_{2}$O. 
\begin{figure}[ht!]
 \centerline{
